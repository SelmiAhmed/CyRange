...
exports.findAll = (req, res) => {
 ...
 exports.findOne = (req, res) => {
  ...
  exports.deleteAll = (req, res) => {
    Question.removeAll(req.params.IDTraining, (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
            message: `Not found Question with id ${req.params.IDTraining}.`
          });
        } else {
          res.status(500).send({
            message: "Could not delete Question with id " + req.params.IDTraining
          });
        }
      } else res.status(201).send({ message: `Question was deleted successfully!` });
    });
